...
DECLARE 
	 @ID			varchar(50)='PC2LCUDAS'		--SMD指定的設備
	,@SMD_JOBID		varchar(10)='17120502'		--SMD指定的批次
	,@CALLING_NUM	varchar(10)=''				--SMD指定的呼出碼(選用)

DECLARE @DEVICE_AREA	varchar(50)='PC2'	--不用改
...
WHERE 1=1
	AND (Field03=@CALLING_NUM or ''=@CALLING_NUM)
...
select 
	...
	,[Status]=0
FROM 
	[192.168.100.65].SMD.dbo.SMD_DCS_LCU_PD PD WITH(nolock)
LEFT JOIN 
	[192.168.100.65].SMD.dbo.SMD_ITEM IT  WITH(nolock)
ON 
	PD.ITEM_ID = IT.ID AND IT.COM_HID = '\PXG\SMD\'
LEFT JOIN 
	[192.168.100.65].SMD.dbo.SMD_INTERFACE_SAVEING_TYPE SAV  WITH(nolock)
ON 
	IT.ID = SAV.ID
LEFT JOIN 
	[192.168.100.65].SMD.dbo.smd_EL_ITEM EL  WITH(nolock)
ON 
	EL.[TYPE]=4 AND EL.VALUE>0 AND PD.CALLING_NUM=EL.CN
WHERE PD.HID = @HID
	AND PD.ID = @SMD_JOBID
